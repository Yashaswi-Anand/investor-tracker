 ...
 * and the time NSE last wrote them.
 *
 * Shares bid and applications come from NSE's live bidDetails; shares
 * offered and the ratio, when NSE publishes them at all, come from its
 * category table. The timestamp is the live one, and it is not decoration:
 * the category table lags badly — measured three days behind on an issue
 * that had gone from twelve times subscribed to twenty-three — so anything
 * sourced from it is the half that can be stale.
 */
export function categoryRows(details) {
  const book = (details || {}).category_bids;
