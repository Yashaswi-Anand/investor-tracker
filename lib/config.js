...
export const SITE = {
  name: "Investor",
  shortName: "Investor",
  // Who actually owns the thing. The product is called Investor; the notice
  // at the foot of every page is a copyright claim, and that belongs to the
  // company rather than to the product's name.
  owner: "Socialriser",
  ...
  // Search Console's HTML-tag verification. Kept as a variable rather than
  // pasted into the layout so proving ownership is a setting rather than a
  // commit — and so the token is not in a public repository. Empty means the
  // tag is simply not emitted; Next omits it rather than writing an empty one.
...
export function organizationLd() {
  return {
    ...
    // schema.org's accountability properties, and the reason /editorial
    // exists: each takes a URL, and all three were empty because there was no
    // document to name. Google asks publishers for exactly these on a site
    // reporting figures people act on.
    publishingPrinciples: `${SITE.url}/editorial`,
    correctionsPolicy: `${SITE.url}/editorial#corrections`,
    ownershipFundingInfo: `${SITE.url}/editorial#funding`,
    // No founder or employee node. This is published by an organisation and
    // maintained by one person who has chosen not to be named; inventing a
    // Person to fill the gap is what Google penalises when it emerges that
