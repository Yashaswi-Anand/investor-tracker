...
/**
 * One-minute ticks into OHLC bars of whatever width is asked for.
 *
 ...
 */
export function toCandles(points, minutes = 5) {
  const width = Math.max(1, minutes) * MINUTE;
...
export async function fetchLive(symbol, board) {
  ...
  // One call, two granularities. The ticks are the same either way, so
  // asking NSE twice to re-bucket the identical minute would be rude.
  const chart = await call(NSE.functions.chart, { symbol: identifier, days: "1D" });
  const ticks = chart?.grapthData;
  const candles = {
    "5m": toCandles(ticks, 5),
    "15m": toCandles(ticks, 15),
  };
